<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:52:"G:\Git\model\web/../app/admin\view\index_action.html";i:1477014431;s:50:"G:\Git\model\web/../app/all/view/index_layout.html";i:1476756500;s:50:"G:\Git\model\web/../app/all/view/index_header.html";i:1478823866;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<script type="text/javascript">
    ...
    $(function(){
        $('#dg').treegrid({
            ...
            url:'<?php echo $ROOT; ?>/admin/action/query',toolbar:'#toolbar',
            columns: [[
                ...
                {field:'image',title:'图标*',width:100, editor:{type:'combobox', options:{url:'<?php echo $ROOT; ?>/all/option/icon',valueField:'name',textField:'name',panelWidth:120,panelHeight:150,required:true,
            ...
            onAfterEdit:function(row,change){
                ...
                var obj={};  //easyui 在1.4.5增加了checkbox属性，导致行直接toJson的时候格式错误。
                obj.id=selected.id;
                obj.description=selected.description;
                obj.action=selected.action;
                obj.rank=selected.rank;
                obj.image=selected.image;
                obj.ismenu=selected.ismenu;
                obj.parentid=selected.parentid;
                obj.shortname=selected.shortname;
                $.post('<?php echo $ROOT; ?>/admin/action/update',{updated:$.toJSON(obj)},function(result){
                    if (result.status==1){
        ...
        //分配部分数据表格
        $('#dg2').datagrid({
            title:'角色分配',idField:'role', striped:'true',pagination:'true',rownumbers:true,singleSelect:true,
            url:'<?php echo $ROOT; ?>/admin/action/role',queryParams:{id:0},toolbar:'#toolbar2',pageSize:5,pageList:[5,10],
            columns: [[
                {field:'role',title:'代码',width:30,align:'center'},
                {field:'name',title:'角色*',width:100,align:'center', editor:{type:'combobox', options:{url:'<?php echo $ROOT; ?>/all/option/role',valueField:'name',textField:'name',panelWidth:160,panelHeight:400,required:true,
            ...
            if (row) {
                $.messager.confirm('确认', '你确定删除此记录么?', function(r){
                    if (r){
                        ...
                        var node={};
                        node.id=row.id;
                        $.post('<?php echo $ROOT; ?>/admin/action/update',{deleted: $.toJSON(node)},function(result){
                            if (result.status==1){
                                $.messager.show({	// show error message
                                    title: '成功',
                                    msg: result.info
        ...
        $("#save2,#menu2_save").click(function(){
            ...
            $.post('<?php echo $ROOT; ?>/admin/action/updaterole',effectRow,function(result){
                if (result.status==1){
